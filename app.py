...
def main():
    ...
    # New Section: Score Explanation for HR
    with st.expander("📊 How is the Hire Score Calculated?", expanded=False):
        st.markdown("""
        **Hire Score Explanation (for HR Professionals)**

        The **Hire Score (0–100%)** is generated by our fine-tuned AI model that compares the Job Description with the candidate’s Resume.

        - **Core AI Model**: Analyzes semantic similarity between JD and Resume.
        - **Additional Intelligence**: Boosts score for strong matches in key areas:
            - Data Science / ML keywords (PyTorch, AWS, Deep Learning, etc.)
            - Senior-level experience and leadership terms
        - **Penalty Applied**: For irrelevant backgrounds (e.g., Accounting, HR, Pure Marketing).

        **Interpretation Guide**:
        - **≥ 75%** → Strong Hire (Highly Recommended)
        - **60–74%** → Good Hire (Consider)
        - **45–59%** → Moderate Fit (Review Manually)
        - **< 45%** → Further Review / Likely Not Suitable
        """)
    ...
    # Analysis (rest of your original code remains unchanged)
    if analyze_btn:
